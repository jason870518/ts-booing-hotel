...
import type { City } from '@/types/Common/common.types';

const { notification } = createDiscreteApi(["notification"]);

export function useCommon(){    //  建立一個與檔名相同的export function
    ...
    let useCustomizationNotification = (type: NotificationType, title: string, content: string, duration?: number) => {
        notification[type]({
            title,
            content,
            duration,
    ...
    onMounted(() => {
        for(let i = 1; i < 60; i++) {
            ...
            if(i < 13) months.value.push(i);
            if(i < 32) days.value.push(i);
        }
    });

    return {    // 沒有setup語法糖所以需要return
        cityData,
